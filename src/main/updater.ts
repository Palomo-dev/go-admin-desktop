...
/**
 * Auto-actualización contra GitHub Releases (ver bloque `publish` en
 * electron-builder.yml). electron-updater compara la versión local con el
 * `latest.yml` publicado y valida el hash del instalador antes de aplicarlo.
 *
 * Solo funciona en la app empaquetada: en desarrollo no hay firma ni versión
 * publicada contra la que comparar.
 */
...
/**
 * Registra los listeners y programa la comprobación periódica de updates.
 */
export function initUpdater(win: BrowserWindow | null): void {
  ...
  // La descarga se dispara manualmente para poder avisar al usuario primero.
...
/**
 * Reinicia la app para aplicar la actualización ya descargada.
 * El agente de impresión se detiene en el hook `before-quit` de index.ts.
 */
