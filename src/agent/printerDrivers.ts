...
async function printViaSystem(printer: PrinterRow, jobType: PrintJobRow['job_type'], payload: PrintJobPayload): Promise<void> {
  ...
  // Normalizar paper_width: aceptar "80mm", "58", 80, null, etc.
  const rawWidth = printer.paper_width;
  let paperWidth: '58mm' | '80mm' = '80mm';
  if (rawWidth) {
    const parsed = String(rawWidth).replace(/mm/i, '').trim();
    const num = parseInt(parsed, 10);
    if (num === 58 || num === 58) paperWidth = '58mm';
    else if (num === 80 || num === 80) paperWidth = '80mm';
    else if (num > 0) paperWidth = num <= 58 ? '58mm' : '80mm';
  }
  console.log(`[printer] printViaSystem: printer="${printer.name}", paper_width="${rawWidth}" -> "${paperWidth}"`);

  // 1. Intentar impresión via Electron (Chromium embebido)
  const electron = tryRequire('electron');
  if (electron && electron.BrowserWindow) {
    try {
      await printViaElectron(html, printer.name, paperWidth);
      return;
    } catch (err: any) {
...
function printViaElectron(html: string, printerName: string, paperWidth: '58mm' | '80mm' = '80mm'): Promise<void> {
  ...
  console.log(`[printer] printViaElectron: printer="${printerName}", paperWidth="${paperWidth}", widthMicrons=${widthMicrons}`);
